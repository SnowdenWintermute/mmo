...
function fillZoneWithTestEntities(zone) {
    ...
    for (let i = 100; i > 0; i--)
        zone.entities[i] = new MobileEntity_1.MobileEntity("entity " + i, new Point_1.Point((0, dist_1.randomInt)(origin.x, bottomRightCorner.x), (0, dist_1.randomInt)(origin.y, bottomRightCorner.y)), (0, dist_1.randomInt)(1, 5), (pos, speed) => {
            pos.x += Math.random() >= 0.5 ? 1 : -1 * speed;
            pos.y += Math.random() >= 0.5 ? 1 : -1 * speed;
        });
}
exports.default = fillZoneWithTestEntities;

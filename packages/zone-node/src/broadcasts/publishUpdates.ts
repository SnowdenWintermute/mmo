...
import getCurrentGhostUpdatesQueue from "./getCurrentGhostUpdatesQueue";
...
import publishGhostUpdateRequests from "./publishGhostUpdateRequests";
...
export default function publishUpdates(publisher: RedisClientType, zone: Zone, engine: Matter.Engine) {
  ...
  const ghostUpdateRequests = getCurrentGhostUpdatesQueue(zone);
  ...
  for (direction in zone.neighboringZonesByDirection) {
    for (const neighborZoneId in zone.neighboringZonesByDirection[direction]) {
      ...
      // if (Object.keys(ghostUpdateRequests).length > 0)
      //   publishGhostUpdateRequests(neighborZoneId, zone, ghostUpdateRequests, publisher);
    }
  }
}
